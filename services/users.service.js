...
class UserService extends Cruds {
  ...
  login(username='', password='') {
    ...
    return new Promise(async function(resolve, reject) {
      try {
        const user = await self.findOne({username: username});
        if (!user) return reject(new Error('User not found'));
        const passwordMatch = await helper.comparePassword(password,
            user.password);
        if (!passwordMatch) {
  ...
  async getUsers() {
    const users = await this.findAll({is_super_admin: false});
